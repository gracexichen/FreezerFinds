...
export function HomeContent() {
  ...
  const [results, setResults] = useState<Store | FrozenFoodExtended | null>(
    null
  );
        ...
        {!loading &&
          ...
          (Array.isArray(results) ? (
            results.map((food) => (
              <div
                key={food.id}
                onClick={() => handleFrozenFoodClick(food.id)}
            ...
            <div
              onClick={() =>
                handleFrozenFoodClick((results as FrozenFoodExtended).id)
              }
              style={{ cursor: "pointer" }}
            >
